...
import { applyVoucher, doDeleteItemCartAction, doUpdateCartAction, setVouchers } from "../../redux/order/orderSlice";
...
const MyCart = () => {
    ...
    const fetchOneAcc = async () => {
        ...
        if (res && res.data) {
            ...
            // Đảm bảo vouchersData được tính toán lại mỗi khi dữ liệu cập nhật
    ...
    const handleApplyVoucher = () => {
        // Lấy thông tin voucher từ dataAccKH
        const voucher = dataAccKH?.[0]?.IdVoucher?.find(item => item.code === voucherCode);
    
        // Nếu tìm thấy voucher và giá trị đơn hàng đủ điều kiện
        if (voucher) {
            const totalOrderValue = order.totalPrice
    
            if (totalOrderValue >= voucher.dieuKien) {
                // Áp dụng voucher
            ...
            notification.error({
                ...
                description: 'Mã voucher bạn nhập không tồn tại.',
            });
        }
    };
            ...
            <div className="rts-navigation-area-breadcrumb bg_light-1">
                ...
                <div className="row">
                    ...
                    <div className="navigator-breadcrumb-wrapper">
                        ...
                        <a className="current">Giỏ hàng của tôi</a>
                    </div>
                    </div>
                </div>
                </div>
            ...
            <div className="container">
                <div className="row g-5">
                        ...
                        {order.carts.map((item, index) => {
                            return (
                                <div className="single-cart-area-list main  item-parent">
                                    <div className="product-main-cart" style={{paddingRight: "20px"}}>
                                        <div className="close section-activation" style={{paddingRight: "20px"}} >
                                            {/* <i className="fa-regular fa-x" /> */}
                                            <Tooltip title="xóa sản phẩm này" color={'green'} key={'green'}>
                                                <RiDeleteBin6Line onClick={() => deleteCartAProduct(item._id, item.sizeDaChon)} style={{color: "red"}} size={25} />                                           
                                            </Tooltip>
                                        </div>
                                    ...
                                    <div className="price">
                                        <p style={{color: "navy"}}>{Math.ceil(item.priceDaChon - (item.priceDaChon * (item.detail.GiamGiaSP / 100))).toLocaleString()}đ</p>
                                    </div>
                                    <div className="quantity">                                        
                                        <InputNumber 
                                            min={1}
                                            value={item.quantity} 
                                            // onChange={() => handleOnChangeInput(item.detail, item.sizeDaChon, item.quantity, item.customerId)}
                                            onChange={(newValue) => handleOnChangeInput(item._id, item.sizeDaChon, newValue)} // Use new value from InputNumber
                                        />
                                    </div>
                                    <div className="subtotal">
                                    <p style={{color: "navy"}}>{((Math.ceil(item.priceDaChon - (item.priceDaChon * (item.detail.GiamGiaSP / 100)) )) * item.quantity).toLocaleString()}đ</p>
                                    </div>
                                </div>
                            )
                        })}
                    ...
                    <div className="col-xl-3 col-lg-12 col-md-12 col-12 order-1 order-xl-2 order-lg-1 order-md-1 order-sm-1">
                        ...
                        <h5 className="title">Thông tin giỏ hàng</h5>
                        <div className="subtotal">
                            <span>Thành Tiền</span>
                            <h6 className="price" style={{color: "navy", fontSize: "17px"}}>{Math.ceil(order.totalPriceChuaGiam).toLocaleString()}đ</h6>
                        </div>
                        <div className="subtotal">
                            <span>Giảm giá</span>
                            <h6 className="price" style={{color: "green", fontSize: "17px"}}> 
                        ...
                        <div className="bottom">
                            ...
                            <h6 className="price" style={{color: "red", fontSize: "17px"}}>{Math.ceil(order.totalPrice).toLocaleString()}đ</h6>
                            </div>
                            <div className="button-area">
                            <button onClick={() => navigate('/checkout')} className="rts-btn btn-primary">Tiến hành đặt hàng</button>
                            </div>
                        </div>
                        </div>
                    </div>
